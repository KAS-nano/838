...
const run = (paths, output, jsonPaths, directory) => new Promise((resolve, reject) => {
...
try {
  ...
  assert.equal((await run([], fromJson, JSON.stringify([b, a]))).status, 0);
  ...
  for (const [index, invalid] of ["broken", "null", "{}", '[123]', '[""]', "[]"].entries()) {
    ...
    assert.notEqual((await run([], target, invalid)).status, 0);
  ...
  for (const [name, inputs] of [
    ...
    assert.notEqual((await run(inputs, target)).status, 0, name);
    assert.equal(existsSync(target), false, `${name}: sem saída parcial`);
  }
  assert.notEqual((await run([a], a)).status, 0);
  ...
  assert.notEqual((await run([], undefined, JSON.stringify([app]), staged)).status, 0);
  assert.equal(existsSync(staged), false, "arquivo compactado ausente: nenhum pacote preparado");
  writeFileSync(`${app}.tar.gz`, "abc");
  assert.equal((await run([], undefined, JSON.stringify([app, b]), staged)).status, 0);
  ...
  assert.notEqual((await run([], undefined, JSON.stringify([app]), staged)).status, 0, "diretório existente rejeitado");
  const collision = path.join(root, "collision");
  assert.notEqual((await run([], undefined, JSON.stringify([app, `${app}.tar.gz`]), collision)).status, 0);
