            ...
            {step === 1 && (
              <div className="space-y-7">
                ...
                <div className="grid gap-5 sm:grid-cols-2">
                  ...
                  <label className="text-sm">Arquitetura da CPU<select aria-label="Arquitetura da CPU" value={profile.cpuArchitecture ?? "x86_64"} onChange={(e) => set("cpuArchitecture", e.target.value as NonNullable<HardwareProfile["cpuArchitecture"]>)} className="mt-2 w-full rounded-xl border border-white/10 bg-surface-raised px-4 py-3 outline-none focus:border-accent/50"><option value="x86_64">x86-64 (Intel/AMD)</option><option value="arm64">ARM64 (Apple Silicon/ARM)</option><option value="other">Outra/não sei</option></select></label>
                  ...
                  <label className="text-sm">Organização da memória<select aria-label="Organização da memória" value={profile.memoryArchitecture ?? "dedicated"} onChange={(e) => set("memoryArchitecture", e.target.value as NonNullable<HardwareProfile["memoryArchitecture"]>)} className="mt-2 w-full rounded-xl border border-white/10 bg-surface-raised px-4 py-3 outline-none focus:border-accent/50"><option value="dedicated">VRAM dedicada + RAM</option><option value="unified">Memória unificada</option><option value="shared">GPU integrada com RAM compartilhada</option></select></label>
                  <label className="text-sm">VRAM dedicada (GB)<input aria-label="VRAM (GB)" aria-invalid={Boolean(errors.vramGb)} type="number" min="0" max="192" value={profile.vramGb} disabled={profile.memoryArchitecture === "unified"} onChange={(e) => set("vramGb", Number(e.target.value))} className="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 outline-none disabled:cursor-not-allowed disabled:opacity-50 focus:border-accent/50"/><span className="mt-1 block text-xs text-muted">{profile.memoryArchitecture === "unified" ? "O cálculo usará a RAM total como orçamento compartilhado." : "Informe a memória exclusiva da GPU."}</span><FieldError message={errors.vramGb}/></label>
            ...
            {step === 3 && (
              <div>
                <div className="grid gap-4 sm:grid-cols-2">
                  <article className="rounded-2xl border border-white/10 bg-black/15 p-5"><Cpu className="h-5 w-5 text-accent" aria-hidden="true"/><h2 className="mt-3 font-medium">Máquina</h2><dl className="mt-4 space-y-2 text-sm text-muted"><div><dt className="inline text-muted">Tipo: </dt><dd className="inline">{profile.deviceType === "desktop" ? "Desktop" : "Notebook"}</dd></div><div><dt className="inline text-muted">CPU: </dt><dd className="inline"><span>{profile.cpu}</span><span className="text-xs"> · {profile.cpuArchitecture ?? "arquitetura não informada"}</span></dd></div><div><dt className="inline text-muted">GPU: </dt><dd className="inline">{profile.gpu}</dd></div><div><dt className="inline text-muted">Memória: </dt><dd className="inline">{profile.memoryArchitecture === "unified" ? `${profile.ramGb} GB unificada` : `${profile.vramGb} GB VRAM · ${profile.ramGb} GB RAM`}</dd></div><div><dt className="inline text-muted">Armazenamento: </dt><dd className="inline">{profile.storageFreeGb} GB livres / {profile.storageTotalGb} GB</dd></div><div><dt className="inline text-muted">Sistema: </dt><dd className="inline">{profile.os}{profile.os === "linux" && profile.distro ? ` · ${profile.distro}` : ""}</dd></div></dl></article>
