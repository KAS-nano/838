...
export default function Recommendations({ catalogModels }: { catalogModels: AiModel[] }) {
  ...
  const list = useMemo(() => recommendHybrid(profile, scenario.objective, catalogModels, seedApiModels, scenario)
    .filter((item) => profile.preference === "both" || item.mode === profile.preference), [catalogModels, profile, scenario]);
  ...
  return <main><div className="page-container recommendation-page"><section>
    ...
    <div className="recommendation-split">
      {(["local", "api"] as const).map((mode) => { const item = list.find((candidate) => candidate.mode === mode); return <section key={mode} className="recommendation-column" aria-labelledby={`${mode}-recommendation-title`}>
        <div className="recommendation-column-heading"><p className="eyebrow">{mode === "local" ? "Recomendação local" : "Recomendação API"}</p><h2 id={`${mode}-recommendation-title`}>{mode === "local" ? "Melhor IA Local para seu PC" : "Melhor experiência via API"}</h2></div>
        {item ? <article className="recommendation-feature panel"><div className="recommendation-feature-body"><span className="recommendation-mode">{item.availabilityType === "both" ? "LOCAL + API" : mode === "local" ? "LOCAL" : "API"}</span><h3>{item.name}</h3>{item.provider && <p className="recommendation-provider">Provider: {item.provider}</p>}{item.quantization && <p className="recommendation-quantization">{item.quantization}</p>}<p className="recommendation-feature-reason">{item.reason}</p><div className="recommendation-metrics">{mode === "local" ? <><div><span>VRAM estimada</span><strong>{item.vramEstimatedGb?.toFixed(1)} GB</strong></div><div><span>RAM estimada</span><strong>{item.ramEstimatedGb?.toFixed(1)} GB</strong></div><div><span>Contexto</span><strong>{item.contextK}K</strong></div><div><span>Velocidade</span><strong>{item.speedRange ? `${item.speedRange.min}–${item.speedRange.max} t/s` : "Indisponível"}</strong></div></> : <><div><span>Velocidade</span><strong>{item.outputTokensPerSecond ? `~${item.outputTokensPerSecond} t/s` : "Benchmark não disponível"}</strong></div><div><span>TTFT</span><strong>{item.ttftMs ? `${item.ttftMs} ms` : "Desconhecido"}</strong></div><div><span>Contexto</span><strong>{item.contextK}K</strong></div><div><span>Preço saída (demonstrativo)</span><strong>{item.priceOutput !== undefined ? `US$ ${item.priceOutput}/M` : "Desconhecido"}</strong></div></>}</div><p className="recommendation-cost">{item.costNote}</p><p className="recommendation-confidence">Confiança: {item.confidence === "high" ? "Alta" : item.confidence === "medium" ? "Média" : "Baixa"} · {item.speedStatus === "unknown" ? "benchmark de velocidade pendente" : item.reasons[0]?.text}</p>{item.scenarioReasons.map((reason) => <p className="scenario-reason" key={reason}>{reason}</p>)}</div><div className="recommendation-feature-footer"><span>{item.availabilityType === "both" ? "LOCAL + API" : mode === "local" ? "LOCAL" : "API"}</span><span>{mode === "local" && item.speedRange ? `${item.speedRange.min}–${item.speedRange.max} t/s` : item.outputTokensPerSecond ? `~${item.outputTokensPerSecond} t/s` : "Sem benchmark"}</span></div></article> : <div className="recommendation-empty panel"><h3>Nenhuma opção disponível</h3><p>Nenhum modelo cadastrado atende à modalidade, ao contexto e ao hardware deste cenário. Ajuste as preferências ou consulte o catálogo.</p></div>}
        {item?.alternatives.length ? <div className="recommendation-alternatives"><h3>Alternativas</h3>{item.alternatives.map((alternative) => <div key={alternative.name} className="recommendation-alternative"><span><strong>{alternative.name}</strong><small>{alternative.detail}</small></span><b>{alternative.score}/100</b></div>)}</div> : null}
        {item && <Link className="recommendation-primary-link" href={mode === "local" ? `/instalar?tool=ollama&model=${item.modelId}` : "/ferramentas"}>{mode === "local" ? "Ver instalação e runtime →" : "Ver provedores e ferramentas →"}</Link>}
      </section>; })}
    </div>
    <p className="scenario-disclaimer">Dados API sem benchmark são exibidos como desconhecidos. Estimativas locais são heurísticas; fontes, datas e confiança devem acompanhar qualquer medição real.</p>
  </section></div></main>;
}
