...
function Dashboard({ catalogModels }: { catalogModels: AiModel[] }) {
  ...
  const modelId = selectedModel ?? params.get("model") ?? catalogModels[0].id;
  const model = catalogModels.find(m => m.id === modelId) ?? catalogModels[0];
  ...
  return <main><div className="page-container space-y-6">
    ...
    <section className="panel hardware-summary" aria-label="Hardware atual">
      ...
      <dl><div><dt>{profile.memoryArchitecture === "unified" ? "Memória unificada" : "VRAM"}</dt><dd>{profile.memoryArchitecture === "unified" ? profile.ramGb : profile.vramGb} GB</dd></div><div><dt>RAM total</dt><dd>{profile.ramGb} GB</dd></div><div><dt>Armazenamento</dt><dd>{profile.storageTotalGb} GB</dd></div><div><dt>Espaço livre</dt><dd>{profile.storageFreeGb} GB</dd></div></dl>
    ...
    <section className="panel dashboard-controls" aria-label="Configuração do modelo">
      <label className="field-label">Modelo<select aria-label="Modelo" value={model.id} onChange={e => { setSelectedModel(e.target.value); const next = catalogModels.find(m => m.id === e.target.value); if (next) setContext(Math.min(contextK, next.contextK)); }} className="control">{catalogModels.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}</select></label>
    ...
    <section className="gauge-grid">
      <SpeedGauge label={profile.memoryArchitecture === "unified" ? "Carga na memória unificada" : "VRAM estimada"} value={memory.vramGb} max={profile.memoryArchitecture === "unified" ? profile.ramGb : profile.vramGb} unit="GB" description={`${memory.gpuLayersPercent}% da carga acelerada · KV ${memory.kvCacheGb} GB`} />
      <SpeedGauge label={profile.memoryArchitecture === "unified" ? "Orçamento unificado estimado" : "RAM estimada"} value={memory.ramGb} max={profile.ramGb} unit="GB" description={profile.memoryArchitecture === "unified" ? "Pesos, KV, runtime e reserva no mesmo conjunto" : memory.gpuLayersPercent < 100 ? "Inclui offload e reserva do sistema" : "Runtime, buffers e reserva do sistema"} />
    ...
    <section className="analysis-grid">
      ...
      <article className="panel p-6"><p className="eyebrow">Origem e confiança</p><div className="mt-3 flex items-center gap-3"><h2 className="text-xl font-medium">Confiança {confidenceLabel.toLowerCase()}</h2><span className="badge">{dataLabel}</span></div><p className="mt-4 text-sm leading-6 text-muted">{perf.note}</p><dl className="mt-5 grid grid-cols-2 gap-4 border-y border-white/10 py-4 text-sm"><div><dt className="text-xs text-muted">Evidências medidas</dt><dd className="mt-2">{perf.evidenceCount}</dd></div><div><dt className="text-xs text-muted">Idade da amostra</dt><dd className="mt-2">{perf.sampleAgeDays === null ? "Sem medição" : `${perf.sampleAgeDays} dias`}</dd></div><div><dt className="text-xs text-muted">Backend provável</dt><dd className="mt-2">{compat.backend}</dd></div><div><dt className="text-xs text-muted">Estimador</dt><dd className="mt-2">v{perf.estimatorVersion}</dd></div></dl><p className="mt-4 text-xs leading-5 text-muted">{perf.confidenceReason}</p>{perf.differingFields.length > 0 && <p className="mt-2 text-xs leading-5 text-muted">Diferenças da evidência: {perf.differingFields.join(", ")}.</p>}<p className="mt-4 text-xs leading-5 text-muted">Memória e compatibilidade são estimativas. As faixas seed não representam um benchmark real do seu computador.</p></article>
    </section>
...
export default function DashboardPage({ catalogModels }: { catalogModels: AiModel[] }) {
  return <Suspense fallback={<main className="page-container">Preparando análise…</main>}><Dashboard catalogModels={catalogModels} /></Suspense>;
}
